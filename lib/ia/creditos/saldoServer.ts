...
async function consumoMesVigente() {
  ...
  const [{ data: ej }, { data: ocr }] = await Promise.all([
    supabaseAdmin.from("ia_ejecuciones").select("tokens_in, tokens_out, costo_estimado").neq("proveedor", "fake").gte("created_at", d0).lt("created_at", d1),
  ...
  return { periodo: mes, tokens_total: tokensTotal, costo_usd: formatoUSD(costoNano), costo_nano: costoNano, limite_tokens_mes: limite, porcentaje_tope: limite > 0 ? Math.round((tokensTotal / limite) * 100) : 0 };
}
...
export async function resumenSaldo() {
  ...
  return {
    ...
    consumo_mes: { periodo: consumoMes.periodo, tokens_total: consumoMes.tokens_total, costo_estimado_usd: consumoMes.costo_usd, porcentaje_tope: consumoMes.porcentaje_tope },
    sincronizacion: {
